...
STUB_TRANSPORT_ENV = "WEDDING_FILM_DEEPSEEK_STUB_TRANSPORT"
...
class HTTPTransport:
    """Talks to the DeepSeek Responses API over HTTPS using only the standard library."""
...
class StubTransport:
    ...
    Selected via WEDDING_FILM_DEEPSEEK_STUB_TRANSPORT so the CLI-seam test suite
    ...
    def send(self, *, api_key: str, payload: dict[str, Any]) -> TransportResponse:
        ...
        if model == STUB_MODEL_INSUFFICIENT_BALANCE:
            return TransportResponse(
                status_code=402,
                body={
                    "error": {
...
class DeepSeekVisionAdapter:
    """Connects the provider-neutral Vision Adapter boundary to the DeepSeek Responses API.

    Never reads or writes the Semantic Catalog; it only turns an Analysis Input into a
    candidate Inference snapshot or a classified failure. DeepSeek's vision model resizes
    every image to roughly 800x800 before inference, so results are coarser than a
    full-resolution adapter on detail-sensitive fields such as quality_flags.
    """
    ...
    def _interpret(self, response: TransportResponse) -> AdapterResult:
        ...
        if response.status_code == 402:
            return self._failure(
                "provider_error",
                False,
                _error_message(response.body, "account balance is exhausted"),
                usage,
                provider_metadata,
            )
