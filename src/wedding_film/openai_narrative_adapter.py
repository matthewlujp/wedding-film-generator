...
_STUB_CANDIDATE: dict[str, Any] = {
    ...
    "moments": [
        {"id": "getting-ready", "prose": "A quiet morning before the ceremony begins."},
        {"id": "joyful-ceremony", "prose": "Vows shared among family and friends."},
    ],
}
...
class StubTransport:
    ...
    scenario, mirroring how the fake adapters fixture model names work.
    """
                    ...
                    "output": [
                        {
                            ...
                            "content": [
                                {
                                    "type": "refusal",
                                    "refusal": "I cannot help write this story.",
                                }
                            ],
                        }
                    ],
...
def _build_payload(
    request: NarrativeRequest, schema: OutputSchema, settings: AdapterSettings
) -> dict[str, Any]:
        ...
        "input": [
            {
                "role": "user",
                "content": [
                    {"type": "input_text", "text": f"{settings.prompt}\n\n{context}"},
                ],
            }
        ],
...
class OpenAINarrativeAdapter:
    """Connects the provider-neutral Narrative Adapter boundary to the OpenAI Responses API.

    Never reads or writes story.md; it only turns a Narrative Request into a
    candidate Story payload or a classified failure.
    """
    ...
    def generate(
        ...
        payload = _build_payload(request, schema, settings)
        try:
    ...
    def _interpret_success(
        ...
        if body.get("status") == "incomplete":
            ...
            return self._failure(
                "invalid_response",
                False,
                f"response was incomplete: {reason_text}",
                usage,
                provider_metadata,
            )
        ...
        if first.get("type") == "refusal":
            ...
            fallback_refusal = "provider refused to generate a Story candidate"
            message = refusal if isinstance(refusal, str) and refusal else fallback_refusal
